...
from shallow_diffusion_tts import get_noise_schedule_list
import numpy as np
import torch
...
class GaussianDiffusion(nn.Module):
    def __init__(
        ...
        if exists(betas):
            ...
            betas = get_noise_schedule_list(
                schedule_mode=args.schedule_type,  # vpsde
        ...
        self.register_buffer(
            "spec_min", torch.FloatTensor(spec_min)[None, None, : args.keep_bins]
        )
        self.register_buffer(
            "spec_max", torch.FloatTensor(spec_max)[None, None, : args.keep_bins]
        )
    ...
    def interpolate(self, x1, x2, t, cond, spk_emb, lam=0.5):
        ...
        for i in tqdm(
            ...
            x = self.p_sample(
                x, torch.full((b,), i, device=device, dtype=torch.long), cond, spk_emb
            )
        x = x[:, 0].transpose(1, 2)
    ...
    def diffuse_trace(self, x_start, mask):
        b, *_, device = *x_start.shape, x_start.device
        trace = [self.norm_spec(x_start).clamp_(-1.0, 1.0) * ~mask.unsqueeze(-1)]
    ...
    def forward(
        ...
        if not infer:
            ...
            # Diffusion
            x_t = self.diffuse_fn(ref_mels, t) * nonpadding  # [B, T_s, 80]
            ...
            for i in tqdm(
                reversed(range(0, t)), desc="ProDiff Teacher sample time step", total=t
            ):
                x = self.p_sample(
